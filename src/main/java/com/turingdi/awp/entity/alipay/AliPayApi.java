...
public class AliPayApi {
     ...
     * Create by quandong
     */
    public static void wapPay(AliAccountInfo aliAccountInfo, PayBizContent payBizContent, HttpServletResponse httpResponse) throws IOException {
        ...
        httpResponse.setContentType("text/html;charset=" + AlipayConstants.CHARSET_UTF8); // 设置文本类型及编码
        httpResponse.getWriter().write(form); // 直接将完整的表单html输出到页面
        httpResponse.getWriter().flush(); // 刷新
    ...
    public static Map<String, String> getRequestParams(HttpServletRequest request) {
        ...
        if(null != request) { // http请求体不为空
            Set<String> paramsKey = request.getParameterMap().keySet(); // 获取请求参数的Key集合
            // 遍历请求参数的Key
            for(String key : paramsKey) {
                params.put(key, request.getParameter(key)); // 将请求参数的Key和Value存放到params中
            }
        }
     ...
     * Create by quandong
     */
    public static void auth(AliAccountInfo aliAccountInfo, HttpServletResponse response, boolean needDetail) throws IOException {
        ...
        response.sendRedirect(authReqUrl); // 通过响应体的重定向方法，向支付宝发送请求，页面上会弹出支付宝授权页面
    }
     ...
     * Create by quandong
     */
    private static AlipayResponse getUserInfo(AliAccountInfo aliAccountInfo, boolean isNeedDetail, HttpServletRequest request) throws IOException {
...
//                        LOG.debug("======================用户真实姓名={}======================", userinfoShareResponse.getRealName());
     ...
     * @throws IOException
     */
    public static AlipayUserInfoShareResponse getUserDetailInfo(AliAccountInfo aliAccountInfo, HttpServletRequest request) throws IOException{
     ...
     * @throws IOException
     */
    public static AlipaySystemOauthTokenResponse getUserId(AliAccountInfo aliAccountInfo, HttpServletRequest request) throws IOException{
