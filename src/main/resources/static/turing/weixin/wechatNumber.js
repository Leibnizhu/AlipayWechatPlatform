...
//      */
//     var returnData = {//主账号选择子账号下拉框，动态加载后加载的数据接收体
...
//             $("#enterpriseName").val(returnData.userId);//下拉框选中userId的选项
//         }
...
//         //传入选中的值，然后加载相应的公众号配置
//         loadingDataByUserId($("#enterpriseName").val());
//     })
//     /**
//      * 下拉框事件，触发重新加载显示数据,根据传入的userId，寻找数据，重新填充进去表单
//      * create by tanjiansheng 2017-08-17
//      */
//     function loadingDataByUserId(userIds) {
...
//                 returnData.commitType = data.commitType;
//                 returnData.operate = data.operate;
//                if("success" == data.operate){
//                    writingDataInForm();//调用方法重新写入数据
...
//         });
//     }
//     //重新写入数据 create by tanjiansheng
//     function writingDataInForm(){
//         returnData.formCommit.val(returnData.commitType);//重新设置提交类型
//         returnData.formName.val(returnData.data.name);//重新设置公众号名称
//         returnData.formAccount.val(returnData.data.account);//重新设置公众号ID
//         returnData.formWxAccount.val(returnData.data.wxAccount);//重新设置微信号
//         returnData.formToken.val(returnData.data.token);//重新设置token
//         returnData.formUrl.val(returnData.data.url);//重新设置url
//         returnData.formAppid.val(returnData.data.appid);//重新设置appid
//         returnData.formAppsecret.val(returnData.data.appsecret);//重新设置appsecret
//         returnData.formId.val(returnData.data.id);//重新设置id
//     }
//
//     $("#wechatInfo").validate({
//         rules: {
...
// 	var name = file.name;
// 	if (name.indexOf("MP_verify") == -1){
//         $("#fileName").show();
//         flag = false;
// 	}
// 	console.log(name);
// });
// $('#fileTxt').on('fileerror', function(event, data, msg) {
//     flag = false;
//     $("#inputFileError").show();
// });
//
// $('#fileTxt').on('fileselect', function(event, numFiles, label) {
